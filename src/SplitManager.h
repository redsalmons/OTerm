...
public:
    ...
    wxWindow* GetFirstLeafWindow() const;
    void Freeze();
    void Thaw();
    ...
    void SetContentPanelChangedCallback(std::function<void(wxWindow*)> callback) { m_contentPanelChangedCallback = callback; }
...
private:
    ...
    std::function<void(wxWindow*)> m_contentPanelChangedCallback;
    ...
    int FindMyNotebookPageIndex(class wxSimplebook* notebook) const;
    bool IsWindowInTree(wxWindow* win) const;
};
